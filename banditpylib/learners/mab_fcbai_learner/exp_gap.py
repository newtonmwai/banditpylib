...
class ExpGap(MABFixedConfidenceBAILearner):
    ...
    def __median_elimination(self) -> Actions:
        ...
        if len(self.__me_active_arms) <= self.__threshold:
            ...
            pulls = math.ceil(
                0.5
                / (self.__me_eps_left**2)
                * (math.log(2 / self.__me_delta_left / len(self.__me_active_arms)))
            )
        else:
            pulls = math.ceil(
                4 / (self.__me_eps_ell**2) * (math.log(3) - self.__me_log_delta_ell)
            )
    ...
    def actions(self, context: Context) -> Actions:
        ...
        if self.__stage == "main_loop":
            ...
            pulls = math.ceil(
                2 / (self.__eps_r**2) * (math.log(2) - self.__log_delta_r)
            )
            for arm_id in self.__active_arms:
                arm_pull = actions.arm_pulls.add()
                arm_pull.arm.id = arm_id
        ...
        if self.__stage == "main_loop":
            ...
            if len(self.__me_active_arms) > self.__threshold:
                ...
                for (arm_id, pseudo_arm) in list(self.__active_arms.items()):
                    if (
                        pseudo_arm.em_mean
                        < self.__active_arms[best_arm_by_me].em_mean - self.__eps_r
                    ):
                ...
                self.__log_delta_r = math.log(
                    (1 - self.confidence) / 50
                ) - 3 * math.log(self.__round)
