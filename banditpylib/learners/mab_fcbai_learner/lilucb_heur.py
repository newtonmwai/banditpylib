...
class LilUCBHeuristic(MABFixedConfidenceBAILearner):
    ...
    def __confidence_radius(self, pulls: int) -> float:
        ...
        return (
            (1 + self.__beta)
            * (1 + math.sqrt(self.__eps))
            * math.sqrt(
                2
                * (1 + self.__eps)
                * math.log(math.log((1 + self.__eps) * pulls) / self.__delta)
                / pulls
            )
        )
    ...
    def __update_ucb(self, arm_id: int):
        ...
        """
        self.__ucb[arm_id] = self.__pseudo_arms[
            arm_id
        ].em_mean + self.__confidence_radius(self.__pseudo_arms[arm_id].total_pulls)
