...
class LinUCB(LinearBanditLearner):
    ...
    def reset(self):
        self.__summation_AtXt = np.zeros(
            (self.__d, 1)
        )  # summation_AtXt: accumulated sum of At * Xt, d x 1
    ...
    def __LinUCB(self) -> np.ndarray:
        ...
        Returns:
          optimistic estimate of arms' real means
        """
        root_beta_t = np.sqrt(self.__lambda_reg) + np.sqrt(
            2 * np.log(1 / self.__delta)
            + self.__d * np.log(1 + (self.__time - 1) / (self.__lambda_reg * self.__d))
        )
        ucb = self.__feature_matrix.T @ self.__theta_hat_t + root_beta_t * np.sqrt(
            (
                self.__feature_matrix.T
                @ np.linalg.pinv(self.__Vt)
