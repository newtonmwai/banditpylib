...
from banditpylib.data_pb2 import Context, Actions, Feedback, ArmPull, ArmFeedback
...
class MultiArmedBandit(Bandit):
    ...
    def __init__(self, arms: List[StochasticArm]):
        if len(arms) < 2:
            raise ValueError(
                "The number of arms is expected at least 2. Got %d." % len(arms)
            )
        self.__arms = arms
    ...
    def _take_action(self, arm_pull: ArmPull) -> ArmFeedback:
        ...
        if arm_id not in range(self.__arm_num):
            raise ValueError(
                "Arm id is expected in the range [0, %d). Got %d."
        ...
        self.__regret += self.__best_arm.mean * pulls - sum(em_rewards)  # type: ignore
