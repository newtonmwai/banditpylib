...
class Protocol(ABC):
        ...
        for learner in learners:
            if not isinstance(learner.running_environment, List):
                if not isinstance(bandit, learner.running_environment):
                    raise Exception(
                        "Learner %s can not recognize environment %s."
    ...
    def play(
        ...
        for learner in self.__learners:
            ...
            logging.info(
                "start %s's play with %s",
                self.__current_learner.name,
                self.__bandit.name,
            )
            ...
            for _ in range(trials):
                result = pool.apply_async(
                    self._one_trial, args=[time_seed()], callback=self.__write_to_file
                )
            ...
            logging.info(
                "%s's play with %s runs %.2f seconds.",
                self.__current_learner.name,
                self.__bandit.name,
